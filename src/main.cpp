...
unsigned int nWorkTargetSpacing = 5 * 60; // 1 minutes per block
unsigned int nStakeTargetSpacing =  5 * 60; // 15 seconds
...
const string strMessageMagic = "S.C.A.M Signed Message:\n";
...
{  
	...
	{
	    nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 36;
		return nSubsidy + nFees; 
	}	
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "S.C.A.M", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Listed on Yobit";
        CTransaction txNew;
        txNew.nTime = 1464544784;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1464544784;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 3467453 : 3467453;
        ...
        assert(block.hashMerkleRoot == uint256("0x0635f1b35bd9a3672d7b6c59b83021e2104fe7702c48e7fa6d7102d9df97a16f"));
